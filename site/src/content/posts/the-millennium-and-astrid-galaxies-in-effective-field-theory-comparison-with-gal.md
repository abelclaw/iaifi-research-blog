...
The researchers worked with two state-of-the-art simulations. **MillenniumTNG (MTNG)** combines the IllustrisTNG galaxy formation model with a gigantic simulation volume. **Astrid** uses a different galaxy formation model — notably a different treatment of **AGN feedback**, the energetic jets and winds driven by supermassive black holes — providing a crucial cross-check on how sensitive results are to uncertain physics.

![Figure 1](/iaifi-research-blog/figures/2412_01888/figure_1.png)

From each simulation, the team selected galaxy samples mimicking real surveys: LRGs matching BOSS selection criteria, and ELGs matching DESI targets. They then applied the **EFT field-level forward model** — a technique operating directly on the full three-dimensional map of galaxy positions, rather than compressing it into summary statistics like the **power spectrum** (a measure of how strongly galaxies cluster at different scales).

This approach cancels much of the **cosmic variance** — the statistical noise from having only one observable Universe — that normally plagues analyses of small simulation volumes. The result: high-precision EFT parameter measurements even from boxes that would otherwise be too small.
...
These parameters were compared against predictions from the standard HOD model for LRGs and the **High Mass Quenched (HMQ) model** — designed specifically for blue, star-forming galaxies that tend to avoid the densest halo environments — for ELGs.

![Figure 2](/iaifi-research-blog/figures/2412_01888/figure_1.png)

The comparison revealed strong agreement, but with important caveats. LRG bias parameters match HOD predictions well. For ELGs, local bias parameters are systematically offset from what an LRG-like model would predict. The team derived an analytic argument explaining why: blue galaxies, which preferentially live at halo outskirts rather than centers, naturally acquire different large-scale bias parameters. This is not a numerical quirk — it reflects real physics about where star-forming galaxies live.

## Why It Matters

The practical stakes are high. DESI is already collecting spectra of tens of millions of ELGs. Extracting cosmological parameters — measuring the Universe's expansion history, testing general relativity, constraining dark energy — requires accurate forward models and well-calibrated priors on bias parameters.

This paper provides a rigorous validation of the HOD-based EFT priors now used in **full-shape analyses** of DESI data — fits to the complete shape of the clustering signal across all scales simultaneously, rather than just broad summary statistics. Getting those priors wrong would bias inferences about the Universe's fundamental properties.

![Figure 3](/iaifi-research-blog/figures/2412_01888/figure_2.png)

One striking finding concerns **fingers-of-God** — the smearing of galaxy clustering along the line of sight from random motions inside halos. LRGs, clustered near dense halo centers, experience strong fingers-of-God effects that push the EFT perturbative framework to its limits. ELGs show weaker such effects, suggesting a larger fraction of ELG data can be modeled perturbatively — potentially extending EFT analyses to smaller scales and tighter cosmological constraints.

The team also found that EFT parameter values depend sensitively on the assumed **baryonic feedback** model — the energetic outflows from supernovae and AGN that reshape how galaxies populate their halos. As surveys push to higher precision, the baryonic physics encoded in simulations will matter more, not less.

Finally, the team deployed **neural density estimation** — a machine learning technique for learning probability distributions from simulation samples — to find HOD parameters that best reproduce field-level clustering in both simulations. This closes the loop: start from hydro simulations, extract EFT parameters, compare with HOD predictions, then use neural inference to identify which HOD configurations best reproduce the full three-dimensional galaxy field. It's a blueprint for combining simulation-based inference with perturbative theory in precision cosmology.

> **Bottom Line:** Red and blue galaxies both fit neatly into the EFT framework, but ELGs play by subtly different rules than LRGs — a difference that traces back to where blue galaxies live in their halos, and one that matters for cosmological analyses of current and future galaxy surveys.

## IAIFI Research Highlights

**Interdisciplinary Research Achievement:** This work bridges hydrodynamical simulations, perturbative field theory, and neural density estimation to build a unified framework connecting galaxy observations to fundamental cosmological parameters.

**Impact on Artificial Intelligence:** The team applied neural density estimation to learn the mapping between HOD parameters and EFT bias parameters from paired simulation samples, demonstrating how machine learning can extract optimal physical models from high-dimensional simulation data.

**Impact on Fundamental Interactions:** By validating EFT bias priors against two independent hydrodynamical simulations with distinct galaxy formation physics, this work strengthens the theoretical foundation for extracting cosmological parameters — including dark energy and the neutrino mass sum — from surveys like DESI.

**Outlook and References:** Future work will extend these comparisons to higher redshifts and smaller scales, and test the framework against actual DESI data; see arXiv:2601.03436 for the full paper.