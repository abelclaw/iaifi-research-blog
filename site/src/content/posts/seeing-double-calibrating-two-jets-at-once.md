...
Imagine trying to measure the weight of two people standing on a seesaw. You could weigh each person separately on a bathroom scale — that's the standard approach. But you could also use the seesaw itself: if it's perfectly balanced, you know both weights must be equal. Even if the seesaw isn't perfectly balanced, the *imbalance* tells you something you couldn't learn from either scale alone.

Physicists at the LHC have been using the bathroom scale approach for decades. This paper shows how to use the seesaw.

At particle accelerators like the Large Hadron Collider, protons smash together at nearly the speed of light, spraying out cascades of particles called **jets** — tight, cone-shaped showers of dozens to hundreds of particles emerging from a single quark or gluon. Measuring jet energies precisely is critical for nearly every physics search, from hunting new particles to pinning down Higgs boson properties. But jets are messy. Detectors add noise, some particles get lost, and reconstructing the true energy from the raw detector signal is genuinely hard.
...
The fits reveal a striking pattern: Λ is consistently *smaller* than σ across all pT ranges studied — exactly the regime where the correlation-improved method helps most.

![Figure 3](/iaifi-research-blog/figures/2402_14067/figure_3.png)

Across the full pT range, the method achieves a **3–5% relative improvement in jet energy resolution**, corresponding to roughly a **35% reduction in the squared resolution**. In experimental physics, where precision campaigns work for years to gain percent-level improvements, that's a significant payoff from a conceptually simple modification. The improved estimates are also **unbiased** — verified in simulation to neither systematically over- nor under-estimate the true jet energy.

## Why It Matters

This work sits at an intersection of statistics and experimental particle physics. The correlation-improved MLE framework is general: it applies whenever you have multiple correlated measurements and some prior knowledge of how they're correlated, even without knowing their individual distributions. The authors already sketch an extension to **boosted resonances** — when a heavy particle like a Z boson or Higgs decays to two nearby jets, the same momentum conservation argument applies in a different energy and geometry regime.

More broadly, this is an example of a growing trend in LHC physics: mining known physical constraints — conservation laws, symmetries, known particle properties — to extract information that naive jet-by-jet analysis leaves on the table. Machine learning has driven much of the recent progress in jet physics, but this paper shows that careful statistical reasoning, without any neural network, can still deliver meaningful gains. The method could also combine with ML-based calibrations, potentially stacking improvements.

The approach is also practical. It requires no exotic hardware, special data collection, or new training infrastructure. It builds on existing per-jet calibrations and adds a simple post-processing step — the kind of improvement large experimental collaborations can realistically adopt.
...
<div style="display:flex;gap:0.75rem;align-items:flex-start;padding:1rem;margin-bottom:0.75rem;border-radius:0.5rem;background:#faf5ff;border:1px solid #e9d5ff;"><img src="/iaifi-research-blog/images/logo-fi-purple.svg" alt="" style="width:32px;height:32px;flex-shrink:0;" /><div><strong style="color:#7b2d8e;">Impact on Fundamental Interactions</strong><br/><span style="color:#374151;">Improving jet energy resolution directly benefits Higgs boson measurements, new physics searches, and QCD studies at the LHC — the core scientific mission of experiments like CMS.</span></div></div>
<div style="display:flex;gap:0.75rem;align-items:flex-start;padding:1rem;margin-bottom:0.75rem;border-radius:0.5rem;background:#ecfdf5;border:1px solid #a7f3d0;"><div><strong style="color:#059669;">Outlook and References</strong><br/><span style="color:#374151;">Future directions include applying this technique to boosted resonances and combining it with ML-based jet calibrations; the full method is available at arXiv:2407.XXXXX (MIT-CTP 5680).</span></div></div>
</div>
