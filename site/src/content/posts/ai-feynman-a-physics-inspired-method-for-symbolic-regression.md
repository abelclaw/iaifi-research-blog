...
2. **Neural network fitting** is the algorithmic workhorse. A standard feedforward neural network is trained on the mystery data. The network itself isn't the answer; it's a probe. Once trained, the algorithm uses it to test for hidden structure.
...
4. **Separability** detection checks whether the function factors into a product or sum of two parts with no shared variables. If $f(x_1, x_2, x_3) = g(x_1) \cdot h(x_2, x_3)$, the problem splits in two. The algorithm tests this by checking whether the network's partial derivatives respect a factored structure.
