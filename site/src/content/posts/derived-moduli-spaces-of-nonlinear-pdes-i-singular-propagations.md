...
4. **Micro-linearize via the Hodge stack.** The linearization sheaf lifts to the **Hodge stack** $X^{\text{Hod}}$, a geometric object that interpolates between two complementary algebraic pictures of the same space. Passing to the associated graded object produces a perfect complex on the cotangent stack $T^*X$. This is the authors' **derived micro-linearization** $\mathbf{R}_\mu(T_{\mathcal{Y}, \phi})$.
