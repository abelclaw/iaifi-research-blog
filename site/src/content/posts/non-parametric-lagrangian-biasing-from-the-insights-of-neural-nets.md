...
But as surveys grow more precise, cracks appear. The polynomial can produce nonsensical results — predicting negative galaxy densities in regions that happen to be emptier than average.

A Harvard-led team has taken a machine-learning approach, using neural networks to learn the bias function directly from simulations without imposing any polynomial form. The network recovers the **halo power spectrum** — a statistical measure of how clustered **dark matter halos** (the massive invisible scaffolding that anchors galaxies) are at different scales — to within 1–2% accuracy, and points toward a surprisingly compact description of how halos form.

> **Key Insight:** By training a neural network on initial cosmic density fields, the researchers discovered that galaxy formation bias — in all its complexity — can be captured by just two dominant directions in a high-dimensional feature space, recovering the halo power spectrum to within 1–2% at large scales.
...
- **Laplacian ∇²δ** — how sharply the density peaks or dips, capturing the "curviness" of the local density landscape
- **Tidal shear G₂** — how much the surrounding gravitational field is trying to stretch or compress that region

These are computed at multiple **smoothing scales** — blurring the density map at different resolutions, from fine-grained to coarse — so each particle carries a whole vector of features.

The neural network learns a weight function: given these initial conditions, how much halo mass forms here? The architecture is simple — three fully connected layers with 20 neurons each — trained on particles from the AbacusSummit simulation suite at redshift z=0.5.

![Figure 1](/iaifi-research-blog/figures/2212_08095/figure_1.png)

A key preprocessing step is **feature orthogonalization**: transforming correlated inputs into independent ones before the network sees them. Density fields at different smoothing scales are highly correlated — the field smoothed over 20 Mpc/h and 30 Mpc/h tell very similar stories. Transforming these into orthogonal components ensures the network isn't processing the same information twice in disguise.
...
- **Three coarsely spaced scales:** The sweet spot — recovers the halo power spectrum to within 1–2% at k < 0.1 h/Mpc
- **More than three scales:** Counterproductive — produces 2–5% *underestimation* of large-scale power with signs of overfitting

That last finding is striking. More information makes things worse. Finely spaced smoothing scales are nearly redundant — they add correlated noise rather than new physics, and that confuses the network.

![Figure 2](/iaifi-research-blog/figures/2212_08095/figure_1.png)

## The Two-Direction Discovery

After training the full network with nine features — three quantities at three scales — the team examined the *gradient* of the learned function, asking which directions in feature space the network actually cares about.

They computed **principal components** of this gradient field, a standard technique for finding the axes of greatest variation in a dataset — like identifying the main axes of an elongated cloud of points. The finding: two components dominate. Two directions, in a nine-dimensional space, capture nearly all the variation in how halos form.
...
Surveys like DESI, Euclid, and the Rubin Observatory are reaching a precision where percent-level accuracy from traditional bias models is no longer sufficient. Understanding galaxy bias at the 1% level is now a practical necessity for extracting cosmological information from these datasets.

This work also illustrates a broader shift: machine learning used not just as a black-box predictor, but as a tool for *discovering* structure in complex physical systems.

The finding that bias compresses to two principal components is potentially profound. It suggests that despite the apparent complexity of galaxy formation, the relevant information in the initial conditions is low-dimensional. If future work can identify what physical process these two directions correspond to — halo mass, assembly history, or something else — it could sharpen theoretical understanding of structure formation.

The multi-scale approach has practical value too. Traditional bias expansions typically operate at a single smoothing scale. This work shows that spanning multiple scales simultaneously — even coarsely — captures physics that single-scale approaches miss. Neural networks handle this naturally, without the memory constraints that limited earlier non-parametric methods to two features at a time.

> **Bottom Line:** A neural network trained on initial cosmic density fields learns galaxy bias in a way that requires just two dominant directions to describe — a surprising compression that recovers the halo power spectrum to 1–2% and suggests that the physics of halo formation, while complex, has a low-dimensional structure waiting to be understood.

<div style="margin-top:2rem;"><h2 style="font-size:1.5rem;font-weight:700;margin-bottom:1rem;">IAIFI Research Highlights</h2>
<div style="display:flex;gap:0.75rem;align-items:flex-start;padding:1rem;margin-bottom:0.75rem;border-radius:0.5rem;background:#f5f5f5;border:1px solid #d4d4d4;"><img src="/iaifi-research-blog/images/logo-fi-black.svg" alt="" style="width:32px;height:32px;flex-shrink:0;" /><div><strong style="color:#1a1a1a;">Interdisciplinary Research Achievement</strong><br/><span style="color:#374151;">This work directly bridges machine learning and large-scale structure cosmology, using neural networks not just as predictors but as scientific instruments that reveal the intrinsic dimensionality of galaxy bias physics.</span></div></div>
<div style="display:flex;gap:0.75rem;align-items:flex-start;padding:1rem;margin-bottom:0.75rem;border-radius:0.5rem;background:#eff6ff;border:1px solid #bfdbfe;"><img src="/iaifi-research-blog/images/logo-ai-blue.svg" alt="" style="width:32px;height:32px;flex-shrink:0;" /><div><strong style="color:#2c5f8a;">Impact on Artificial Intelligence</strong><br/><span style="color:#374151;">The study demonstrates how principal component analysis of learned gradients can both improve neural network performance and expose interpretable structure in high-dimensional astrophysical feature spaces.</span></div></div>
<div style="display:flex;gap:0.75rem;align-items:flex-start;padding:1rem;margin-bottom:0.75rem;border-radius:0.5rem;background:#faf5ff;border:1px solid #e9d5ff;"><img src="/iaifi-research-blog/images/logo-fi-purple.svg" alt="" style="width:32px;height:32px;flex-shrink:0;" /><div><strong style="color:#7b2d8e;">Impact on Fundamental Interactions</strong><br/><span style="color:#374151;">By achieving 1–2% recovery of the halo power spectrum in a non-parametric framework, this work advances the precision modeling needed to constrain inflation physics and dark energy from upcoming galaxy surveys.</span></div></div>
<div style="display:flex;gap:0.75rem;align-items:flex-start;padding:1rem;margin-bottom:0.75rem;border-radius:0.5rem;background:#ecfdf5;border:1px solid #a7f3d0;"><div><strong style="color:#059669;">Outlook and References</strong><br/><span style="color:#374151;">Future work will extend this framework to observable galaxies and explore whether the two dominant principal components can be linked to known physical quantities; the paper is available on arXiv as part of the AbacusSummit simulation analysis series.</span></div></div>
</div>
