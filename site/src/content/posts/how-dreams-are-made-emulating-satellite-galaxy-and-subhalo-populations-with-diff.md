...
Now, a team led by Tri Nguyen at MIT — with collaborators across IAIFI, Princeton, Harvard, and Flatiron — has built something that breaks that tradeoff: **NeHOD**, a neural network that generates realistic galaxy populations with hydrodynamic fidelity at a fraction of the computational cost.
...
The traditional workhorse for placing galaxies in simulations is the **Halo Occupation Distribution (HOD)** — statistical rules that say, roughly, "a halo of this mass probably hosts this many galaxies." It's fast, but it ignores spatial detail and the influence of **baryonic physics** — processes involving ordinary matter, like supernova explosions blowing gas out of galaxies. At the other extreme, hydrodynamic simulations solve full fluid dynamics equations simultaneously for gas, stars, and dark matter — beautiful physics, but a single run can take millions of CPU hours.

NeHOD sits between these extremes, learning the mapping from dark matter halo properties to galaxy populations by training on 1,024 high-resolution **zoom-in simulations** — focused runs that concentrate computing power on a single halo — from the **DREAMS project** (the TNG-Warm DM suite). These simulations all model Milky Way-mass halos while varying two key parameters: warm dark matter particle mass and astrophysical feedback strength.

![Figure 1](/iaifi-research-blog/figures/2409_02980/figure_1.png)

The architecture has two interlocking pieces:

1. **A Transformer encoder** — originally developed for language processing — reads the dark matter halo as a cloud of particle positions and compresses it into a compact summary of the halo's structure, mass profile, and shape.
2. **A variational diffusion model** — which learns to turn random noise into structured data, step by step — generates the satellite galaxy population conditioned on that summary and the simulation parameters. Starting from noise, it iteratively refines a set of points into realistic galaxy positions, velocities, stellar masses, and concentration parameters.

By treating galaxies as a **point cloud** — individual points in space rather than values on a fixed grid — NeHOD avoids the resolution limits that constrain grid-based approaches. It resolves spatial scales down to the simulation's native resolution, critical for studying small satellite galaxies orbiting close to their host.

![Figure 3](/iaifi-research-blog/figures/2409_02980/figure_2.png)

NeHOD correctly reproduces the key statistics of satellite galaxy populations: the **subhalo mass function** (the count of satellite halos at each mass), the **stellar-to-halo mass relation**, the **concentration-mass relation**, and **spatial clustering statistics** — all as a function of the simulation parameters. When warm dark matter is lighter and suppresses small-scale structure, NeHOD generates fewer low-mass satellites. When feedback parameters change, it adjusts stellar masses accordingly.

Crucially, the model captures **correlations between properties** that simpler models miss. In a real galaxy system, a satellite's position, velocity, mass, and concentration co-evolved under the same gravitational and baryonic forces. The diffusion model learns these joint distributions implicitly, without requiring the researcher to specify them.

![Figure 5](/iaifi-research-blog/figures/2409_02980/figure_3.png)

The computational payoff is significant: populating a halo with NeHOD takes seconds on a GPU; the equivalent hydrodynamic simulation takes months on a supercomputer cluster.

## Why It Matters

This work sits at the crossroads of two major thrusts in modern cosmology. Surveys like SAGA and ELVES are already cataloging satellite galaxies around Milky Way analogs, with Rubin and Roman set to expand that census by orders of magnitude. Meanwhile, physicists are hunting for signatures of **warm dark matter** — a hypothetical variant whose fast-moving particles would smooth out small clumps, leaving detectable gaps in satellite galaxy counts. Detecting that signal requires running thousands of forward models across wide parameter spaces, something only a fast emulator like NeHOD makes feasible.

Accurate subhalo populations also open doors for **strong gravitational lensing** studies — where a massive galaxy bends light from a distant source into arcs or rings, with the exact pattern depending on the detailed matter distribution including small satellites. They are equally essential for **satellite kinematics** — using dwarf galaxy motions to infer the mass profile of their host halo. The authors flag both as target applications.

The broader lesson may be just as important: point-cloud diffusion models, borrowed from machine learning's work on 3D object generation, turn out to be a natural fit for astrophysical structures that are inherently sparse and irregular in space.
...
<div style="display:flex;gap:0.75rem;align-items:flex-start;padding:1rem;margin-bottom:0.75rem;border-radius:0.5rem;background:#f5f5f5;border:1px solid #d4d4d4;"><img src="/iaifi-research-blog/images/logo-fi-black.svg" alt="" style="width:32px;height:32px;flex-shrink:0;" /><div><strong style="color:#1a1a1a;">Interdisciplinary Research Achievement</strong><br/><span style="color:#374151;">This work directly bridges modern deep learning — variational diffusion models and Transformers from computer vision and NLP — with astrophysical simulation, demonstrating that point-cloud generative models can emulate complex hydrodynamic outputs with high fidelity.</span></div></div>
