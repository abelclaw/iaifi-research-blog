...
- **Interdisciplinary Research Achievement:** This work unifies p-adic string theory and arithmetic geometry by proving that the Néron local height function on the Tate curve is the large-$p$ limit of a non-Archimedean CFT two-point function, connecting number theory and quantum field theory in a single result.

- **Impact on Artificial Intelligence:** The Bruhat-Tits tree filtration developed here, which discretizes infinite-dimensional p-adic spaces via tree truncation, provides a rigorous mathematical framework that may inform hierarchical and tree-structured representations in machine learning.

- **Impact on Fundamental Interactions:** This paper provides the first explicit Green's function for a p-adic string worldsheet at genus one, unlocking computation of p-adic closed string torus amplitudes and bringing the adelic formulation of string theory a significant step closer to reality.

- **Outlook and References:** Future work targets p-adic string torus amplitudes computed from this Green's function and a full adelic product formula at genus one. The paper is available at [arXiv:2512.24935](https://arxiv.org/abs/2512.24935).
