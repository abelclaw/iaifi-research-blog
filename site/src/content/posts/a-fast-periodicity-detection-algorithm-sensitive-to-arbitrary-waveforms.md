...
Imagine finding a heartbeat in a crowd of 1.5 billion people, simultaneously, in real time, using nothing but faint flickering light. That's what modern astronomers face as sky surveys like the **Zwicky Transient Facility (ZTF)** generate torrents of data on billions of stars, each observed hundreds or thousands of times. Hidden in those brightness records are pulsing stars, eclipsing binaries, and exotic objects feeding off companion stars. But you have to detect their rhythms fast enough to keep pace with the data.
...
Douglas Finkbeiner (Harvard), Thomas Prince, and Samuel Whitebook (Caltech) developed **Fast Periodicity Weighting (FPW)**: an algorithm that detects periodic signals of *any* waveform shape, derived from statistical first principles, and fast enough to process over a billion objects on modern hardware.
...
The **Vera Rubin Observatory's Legacy Survey of Space and Time (LSST)** will push object counts higher and observation cadences denser. Algorithms adequate for millions of sources become bottlenecks at billions. FPW was built with this scaling problem in mind: its explicit target is ZTF's full 1.5-billion-object dataset, and it's already running in production.
...
A companion paper will provide the full Gaussian Process derivation, quantitative comparisons across waveform types, and rigorous statistical characterization, laying the groundwork for FPW as a community standard.
