...
Imagine trying to understand how a calculator works by only looking at its output — no buttons, no display, just the answer. That's roughly what researchers face when trying to decode how a neural network — the software architecture behind modern AI — actually solves a problem. The field of **mechanistic interpretability** is dedicated to cracking open these networks and reading the algorithm hidden inside.

There's a catch: most existing work starts with a known algorithm and builds a network specifically designed to reproduce it. That guarantees interpretability, but also guarantees you'll never discover anything genuinely new.
...
> **Key Insight:** Hypernetworks can systematically explore the space of interpretable algorithms, discovering computational strategies that humans never thought to try — yet can still understand in hindsight.
...
- **The double-sided algorithm**: The expected one. Build an absolute value from two ReLU neurons — one firing for positive inputs, one for negative — then sum across all dimensions. Clean, familiar, unsurprising.
- **The pudding algorithm**: Unexpected. This method exploits a mathematical identity involving a limit as a constant approaches infinity, squeezing the L1 computation through a different algebraic path. It comes in two signed variants — "positive pudding" and "negative pudding" — which look visually distinct in the force-directed drawings.
- **The convexity algorithm**: Also unexpected. This one leverages the convexity properties of the absolute value function through a completely different computational route.
