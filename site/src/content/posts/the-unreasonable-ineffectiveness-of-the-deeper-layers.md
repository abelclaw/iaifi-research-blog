...
To measure this, the researchers compute the **angular distance** between the input and output of each layer: how different the two snapshots of information are, expressed as an angle in mathematical space. Think of it like comparing two arrows. If they point in nearly the same direction, the layer barely changed anything.
...
There are two ways to read this result. The optimistic interpretation: these models have more capacity than current training methods know how to fill. The deeper layers are undertrained, not inherently useless, which signals that better training recipes, longer runs, or different architectures could unlock substantial gains without adding a single parameter.

The more sobering interpretation: shallow layers do most of the heavy lifting when it comes to storing factual knowledge, and depth beyond a certain point provides diminishing returns regardless of training. Either way, the trillion-parameter scaling race may need to reckon with this.

On the practical side, compressed models that retain most of a full model's capability but run in half the memory open doors for deployment on commodity hardware, faster inference, and sharply reduced costs. The single-GPU fine-tuning requirement means this is accessible to the broader research community, not just labs with massive clusters.

Open questions remain. Does this apply to reasoning-focused models trained with reinforcement learning? Are there tasks, like complex multi-step math or long-form reasoning, where the deeper layers actually matter? And can training procedures be redesigned from the start to use depth more efficiently?
