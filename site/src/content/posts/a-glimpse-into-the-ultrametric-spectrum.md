...
This odd idea is the mathematical heart of a decades-old proposal for reinventing string theory from the ground up. Now, two theorists have taken a significant step toward making that proposal work, uncovering a hidden spectrum that behaves, against all odds, just like ordinary strings.

String theory's most celebrated prediction is an infinite tower of vibrating states — each successive mode heavier than the last, arranged with beautiful mathematical regularity. That regularity has a deep thermodynamic consequence: the number of ways to pack a given amount of energy into string vibrations follows a formula Hardy and Ramanujan proved over a century ago. It's an entropy fingerprint, a signature that distinguishes string theory from nearly everything else.

The question An Huang and Christian Jepsen set out to answer: can you reconstruct that fingerprint in a world built not on ordinary numbers, but on ***p*-adic numbers** — an alternative number system where "closeness" is measured by divisibility rather than size, producing a universe with a naturally tree-like, hierarchical structure?
...
The researchers built their analysis around a central analogy. A string, discretized into a chain of beads, is mathematically equivalent to an infinite path graph — a chain where every interior vertex has exactly two neighbors. The **Bruhat-Tits tree** (also known as the Bethe lattice) is the natural *p*-adic analog: a branching tree where every vertex has *p*+1 neighbors. Think of it as an infinite family tree where every member has exactly the same number of descendants, forever. The question is whether quantizing vibrations on this tree-graph produces the same thermodynamic behavior as quantizing vibrations on a string.

The first answer is no. When Huang and Jepsen associated quantum harmonic oscillators to the **normal modes** — the independent vibration patterns of the whole system, like the distinct notes a plucked guitar string can ring out — of finite tree-graphs, they found results that depend critically on boundary conditions:
...
So the simple tree approach fails. But the Neumann case hinted at something: even though the overall scaling is wrong, the spectrum has an interesting structure — exponentially spaced levels with exponentially growing degeneracies. Could these two effects balance?

![Figure 1](/iaifi-research-blog/figures/2601_03738/figure_1.png)

The key breakthrough came when the authors shifted focus from tree-graphs to the ***p*-adic circle** — written U*p*, the set of *p*-adic numbers with unit norm, serving as the hierarchical world's equivalent of the ordinary unit circle that underlies standard string quantization. The relevant operator is the **Vladimirov derivative**, the *p*-adic analog of ordinary differentiation — a mathematically rigorous way to measure how quickly functions change in an ultrametric space. Computing its eigenvalues on U*p* yields concrete spectra for small primes:
...
The pattern is striking. Energies grow roughly like *p*^*n*, while degeneracies grow like (*p*−1)·*p*^(*n*−1). The ratio — the density of states — stays controlled in just the right way.

![Figure 2](/iaifi-research-blog/figures/2601_03738/figure_2.png)

To extract the thermodynamics, the researchers computed the **partition function** — the fundamental accounting tool of statistical mechanics, tallying all possible states weighted by their thermal probability. Extracting high-temperature asymptotics required care: both energies and degeneracies grow exponentially, so standard approximation schemes break down. The authors developed a systematic expansion showing that entropy scales as the square root of energy, exactly as in the ordinary non-relativistic string.

The catch is a beautiful one. The entropy doesn't simply scale like √*E* — it oscillates. The leading behavior is Hardy-Ramanujan-like, but modulated by log-periodic fluctuations that repeat every time the energy increases by a fixed multiplicative factor. This is a hallmark of **discrete scale invariance**: the ultrametric world is hierarchical by construction, and that hierarchy leaves an indelible mark on the thermodynamics.

![Figure 3](/iaifi-research-blog/figures/2601_03738/figure_3.png)

The authors also showed that the same spectrum arises as the eigenspectrum of the **Neumann-to-Dirichlet operator** on U*p* — a boundary-value operator that maps how steeply a function slopes at an edge to the function's value there. This connection provides an independent derivation of the ultrametric string spectrum, reinforcing its mathematical naturality.

## Why It Matters

*P*-adic string theory has been discussed since the 1980s, but the original Freund-Olson formulation contained only a single state — the tachyon — making it phenomenologically barren. Huang and Jepsen's construction points toward a richer ultrametric string theory with a genuine tower of states, bringing it substantially closer to the real thing.

The log-periodic fluctuations are not a bug — they are a feature. Such oscillations appear in critical phenomena on hierarchical lattices, in systems near catastrophic failure, and in certain quantum gravity models. Their presence in the ultrametric string entropy reflects a deep mathematical fact: discrete scale invariance leaves oscillatory fingerprints wherever it goes. Future work will need to address whether these oscillations survive in a fully Lorentz-covariant setting, and whether they carry observable consequences.

The experimental connection is also worth noting. Within the past five years, researchers have engineered spin systems that interpolate between ordinary linear order and *p*-adic (2-adic) hierarchical order. As experimental control of ultrametric systems improves, the thermodynamic signatures described here may become directly testable.
...
<div style="display:flex;gap:0.75rem;align-items:flex-start;padding:1rem;margin-bottom:0.75rem;border-radius:0.5rem;background:#f5f5f5;border:1px solid #d4d4d4;"><img src="/iaifi-research-blog/images/logo-fi-black.svg" alt="" style="width:32px;height:32px;flex-shrink:0;" /><div><strong style="color:#1a1a1a;">Interdisciplinary Research Achievement</strong><br/><span style="color:#374151;">This work bridges number theory (*p*-adic analysis, the Vladimirov derivative) with theoretical physics (string thermodynamics, the Hardy-Ramanujan formula), demonstrating that tools from pure mathematics can illuminate the deep structure of candidate quantum gravity theories.</span></div></div>
...
<div style="display:flex;gap:0.75rem;align-items:flex-start;padding:1rem;margin-bottom:0.75rem;border-radius:0.5rem;background:#ecfdf5;border:1px solid #a7f3d0;"><div><strong style="color:#059669;">Outlook and References</strong><br/><span style="color:#374151;">Future work will explore Lorentz-covariant generalizations and the role of log-periodic oscillations in full quantum ultrametric string theory; the paper is available at arXiv:2501.04786.</span></div></div>
</div>
