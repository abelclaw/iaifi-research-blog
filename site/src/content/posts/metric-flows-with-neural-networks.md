...
For decades, physicists and mathematicians have tried to approximate these elusive **Riemannian metrics** — the mathematical rulers that measure distances and angles on curved spaces. Classical methods like Donaldson's algorithm can get close, but computing a single approximation might take years. Recently, neural networks cracked that barrier: a modern laptop running the right network achieves the same result in thirty minutes. But *why* neural networks work so much better has remained mysterious — until now.

Researchers James Halverson and Fabian Ruehle at Northeastern University and IAIFI have developed a complete mathematical theory explaining how neural networks flow through the space of geometric metrics, unifying deep learning theory with the mathematics of curved spaces — and revealing why neural networks decisively outperform classical approaches like **Ricci flow**, a technique that smooths out geometric irregularities the way heat spreads through a material.
...
With careful architectural choices — networks that couple metric values only to nearby points — and a specific loss function, the authors show that the infinite-width, fixed-kernel flow reproduces **Perelman's formulation of Ricci flow**: the same geometric evolution equation Grigori Perelman used in his celebrated 2003 proof of the Poincaré conjecture. Perelman reformulated Ricci flow as minimizing a single geometric quantity called the F-functional, and the authors show this corresponds exactly to a specific neural network architecture in the infinite-width limit. Ricci flow — one of the greatest tools in modern geometry — is secretly a special case of neural network metric flow.

But those special conditions — infinite width, fixed kernel, local couplings — look highly restrictive from a machine learning perspective. And the experiments confirm it.

The team tested fixed-kernel versus evolving-kernel networks on the quintic threefold, a classic Calabi-Yau manifold. The results were stark: fixed-kernel networks, mimicking infinite-width Ricci-flow dynamics, learned the metric poorly, with errors staying high. Finite-width networks with an evolving metric-NTK converged to accurate metrics. Measuring the kernel at different training stages confirmed the picture — for well-trained finite-width networks, the kernel changed substantially from initialization to convergence; for fixed-kernel networks, it stayed frozen, and the results suffered accordingly.

![Figure 2](/iaifi-research-blog/figures/2310_19870/figure_2.png)

## Why It Matters

On the physics side, accurate numerical Calabi-Yau metrics are essential for extracting predictions from string theory compactifications — determining particle masses, coupling constants, and the structure of the Standard Model. Every advance in numerical methods is a step toward connecting string theory to observable physics.

On the mathematics side, the work reveals that Ricci flow is embedded inside a much richer family of flows parameterized by neural network architectures. This opens a new landscape of geometric flows with no classical names yet — generated by different network designs and loss functions. Some might converge faster, handle more complex geometries, or possess properties that mathematicians would find genuinely useful.

![Figure 3](/iaifi-research-blog/figures/2310_19870/figure_3.png)

On the AI side, the theory provides a concrete, geometric case where the NTK framework makes falsifiable, experimentally verified predictions. It demonstrates that feature learning — the ability of finite-width networks to evolve their internal representations — provides a quantifiable advantage over kernel methods, measured in geometric error rather than classification accuracy. As neural networks are applied to more problems in fundamental science, this kind of theoretical grounding becomes increasingly important.

Open questions abound. Can different neural architectures realize other classical geometric flows? What is the full landscape of fixed points for neural network metric flows? Can these methods scale to higher-dimensional Calabi-Yau manifolds with more complex topology, potentially unlocking new regions of the string theory landscape?
...
<div style="display:flex;gap:0.75rem;align-items:flex-start;padding:1rem;margin-bottom:0.75rem;border-radius:0.5rem;background:#f5f5f5;border:1px solid #d4d4d4;"><img src="/iaifi-research-blog/images/logo-fi-black.svg" alt="" style="width:32px;height:32px;flex-shrink:0;" /><div><strong style="color:#1a1a1a;">Interdisciplinary Research Achievement</strong><br/><span style="color:#374151;">This work builds a unified theory connecting neural network gradient descent with Riemannian geometry and differential topology, demonstrating that Perelman's Ricci flow is a special case of neural network metric flow.</span></div></div>
...
<div style="display:flex;gap:0.75rem;align-items:flex-start;padding:1rem;margin-bottom:0.75rem;border-radius:0.5rem;background:#ecfdf5;border:1px solid #a7f3d0;"><div><strong style="color:#059669;">Outlook and References</strong><br/><span style="color:#374151;">Future work could explore whether tailored neural architectures can realize other classical geometric flows or discover entirely new ones; the paper is available at arXiv:2310.16526.</span></div></div>
</div>
