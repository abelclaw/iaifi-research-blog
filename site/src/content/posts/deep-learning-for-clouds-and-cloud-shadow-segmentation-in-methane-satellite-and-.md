...
- **U-Net** (a neural network originally developed for medical image segmentation) analyzes full images rather than individual pixels. It passes information down through the network to extract features, then back up to reconstruct spatial detail, producing clean, coherent masks that respect the physical extent of cloud systems.
- **SCAN (Spectral Channel Attention Network)** applies **attention mechanisms** — a technique that lets the model focus on the most informative inputs and downweight the rest — to the spectral dimension, learning which wavelength bands carry the strongest signal for each pixel. Where U-Net wins on spatial coherence, SCAN wins on boundary sharpness. SCAN actually outperforms U-Net on MethaneSAT specifically, suggesting that at coarser spatial resolutions, spectral discrimination matters more than spatial reasoning.

The researchers combined both models using a **convolutional ensemble** — a small neural network trained to intelligently merge U-Net and SCAN predictions. This ensemble achieved the best results on both platforms: an **F1 score** of 78.50% (±3.08%) on MethaneAIR and 78.80% (±1.28%) on MethaneSAT, where F1 balances catching real clouds against falsely flagging clear sky. Against conventional methods, that's improvements of 2% and 10% respectively. The larger gain on MethaneSAT shows just how much conventional approaches struggled with the satellite's lower resolution.

![Figure 2](/iaifi-research-blog/figures/2509_19665/figure_2.png)

## Why It Matters

The immediate payoff is operational. At 4.1 milliseconds per 1,000 square kilometers, this system can keep pace with satellite data volumes without becoming a bottleneck. MethaneSAT covers massive stretches of oil and gas basins and agricultural regions globally. Screening that data for clouds in near-real-time is a prerequisite for everything downstream, from emission maps to regulatory accountability reports.

The deeper contribution is methodological. This work establishes that different deep learning architectures encode different types of information about hyperspectral scenes — U-Net captures geometry, SCAN captures spectral identity — and that combining them recovers signal that neither captures alone. That principle extends well beyond methane: any mission doing gas retrieval from hyperspectral data faces the same cloud contamination problem. With publicly released code and labeled datasets, this pipeline gives the next generation of hyperspectral missions a working foundation to build from.
