...
A team of researchers led by Jasmine Shone and Zhening Li asked a simple but radical question: what if we just used a different measuring stick? Their answer, **Beyond I-Con**, shows that swapping out KL divergence for bounded alternatives doesn't just work — it sets new state-of-the-art benchmarks across three major tasks in representation learning.
...
The critical difference: KL divergence explodes to infinity as the learned probability of a transition approaches zero. TV, JSD, and Hellinger all stay bounded. That boundedness means the training algorithm isn't catastrophically penalized for placing dissimilar things far apart — which is exactly what good representation learning should do.

![Figure 1](/iaifi-research-blog/figures/2509_04734/figure_1.png)

The dimensionality reduction results make this viscerally clear. Running **SNE** (Stochastic Neighbor Embedding — an algorithm that compresses high-dimensional data into a 2D map) on **CIFAR-10** images (a standard benchmark of labeled photographs) with KL divergence produces the classic "crowding problem": ten color-coded classes smeared into a single overlapping blob. Swap in TV, JSD, or Hellinger, and the same algorithm produces ten cleanly separated islands. Same architecture, same data, same training procedure. Just a different measuring stick.

For **unsupervised clustering** — grouping images by similarity without any pre-existing labels — the team modified the PMI algorithm to cluster DINO ViT embeddings on **ImageNet-1K**, one of the most demanding visual benchmarks in the field (over a million photographs). Using TV distance, they beat the previous state-of-the-art on both ViT-B/14 and ViT-L/14 models, hitting 65.13% and 68.40% Hungarian Accuracy (how well predicted clusters match true categories) respectively.

For **supervised contrastive learning** — training a model to pull similar examples together and push different ones apart using labeled data — they trained ResNet-50 on CIFAR-10. JSD improved linear probe accuracy (how well a simple downstream classifier can read off the learned representations) from 90.03% to 90.84%, a consistent gain across five random seeds. Small numbers, but on a mature benchmark, meaningful ones.

![Figure 2](/iaifi-research-blog/figures/2509_04734/figure_2.png)

The gradient dynamics tell the same story. With KL-based losses, training signals show violent spikes early in optimization — the signature of an unstable landscape. The bounded divergences train smoothly. The choice of loss function shapes the entire training trajectory.

## Why It Matters

This work matters beyond its benchmark numbers because it challenges a near-universal assumption. Contrastive learning methods like SimCLR, MoCo, and CLIP — which power much of modern visual AI — were designed around KL-adjacent objectives without systematic justification. Beyond I-Con shows that the choice of divergence is a genuine design axis, one that has been essentially unexplored. If even small substitutions (KL → JSD) reliably improve performance, the field may be sitting on a large and untapped design space.

There's also a conceptual payoff for researchers building new methods. I-Con showed that dozens of apparently different algorithms shared a hidden unity. Beyond I-Con adds a new dimension to that map: not just "which distributions are you comparing?" but "with what yardstick?" Future methods could be designed from the top down — choosing the divergence that best matches the geometry of the task — rather than inheriting KL by default.
