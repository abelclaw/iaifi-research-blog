...
The researchers applied **AdS/CFT** — a framework linking gravity inside a curved region of spacetime to a different type of physics on that region's surface — in a radically simplified mathematical universe built on **p-adic numbers**. Think of AdS/CFT as decoding the interior of a snow globe by studying only its glass shell. P-adic numbers are a bizarre alternative number system where "closeness" works differently than on the real number line: two numbers can be "nearby" even when they appear far apart by ordinary arithmetic.

The target: **finite-temperature CFTs**. A conformal field theory (CFT) is a mathematical model for physical systems (like materials at the edge of a phase transition) that look statistically the same at every scale. At zero temperature, the math stays manageable. Switch on a temperature, and contributions from every type of excitation pile on; the math explodes in complexity.

The researchers showed that p-adic AdS/CFT, built on a discrete tree-like geometry called the **Tate curve**, provides a clean, tractable toy model that reproduces the qualitative structure of real finite-temperature CFTs — with computations that actually close.

> **Key Insight:** By replacing continuous curved space with a discrete tree structure and real numbers with p-adic numbers, this paper makes the notoriously hard problem of finite-temperature CFT correlators suddenly computable — and the answers parallel the real-world formulas in striking ways.

## How It Works

The story begins with the **Bruhat-Tits tree** — the p-adic analog of Anti-de Sitter (AdS) space. In ordinary AdS/CFT, the bulk is a smooth hyperbolic space and the boundary hosts a conformal field theory. In the p-adic version, the bulk is an infinite tree: each vertex connects to p+1 neighbors, branching outward forever, with the "boundary" living at the tree's infinite ends. This discrete structure eliminates derivative operators entirely (a massive technical simplification) while preserving the essential holographic logic.

To introduce temperature, you must **compactify** a direction: wrap one dimension into a finite loop, the way rolling a strip of paper into a cylinder creates a circle. In real AdS/CFT, a thermal CFT lives on a cylinder S¹ × ℝ, and the bulk dual involves a black hole geometry. In the p-adic world, the analogous construction is the Tate curve — the Bruhat-Tits tree with vertices glued by a discrete symmetry, wrapping the tree around a finite thermal cycle. The result is a central loop ("the thermal cycle") embedded in a tree, with branches hanging off each vertex.
