...
SBL appears across neuroscience and imaging — reconstructing brain activity in MRI, inferring neural firing from calcium imaging, estimating radio signal directions. Its defining advantage over methods like LASSO is that it doesn't just return a single answer. It returns a full probability distribution over all possible answers. That honest accounting of uncertainty is indispensable for downstream decisions.

The problem? SBL has a computational bottleneck baked into its core. Every update step requires working with a **covariance matrix** — a grid tracking how every unknown in your signal relates to every other. For *D* unknowns, this grid has *D²* entries and requires *O(D³)* operations to manipulate. Double the problem size, and computation becomes eight times slower.

At real-world scales — a brain scan with tens of thousands of voxels — SBL grinds to a halt. A team from Harvard and MIT built a fix: **CoFEM**.
...
CoFEM's insight is that you don't need the whole matrix. You only need two quantities from it: the **posterior mean** (your best single-point estimate of the signal) and the **diagonal of the covariance** (a compact list of per-unknown uncertainties). Both can be estimated without ever constructing the full matrix.

Here's how CoFEM does it:

1. **Solve linear systems instead of inverting matrices.** Rather than directly computing Σ = (Φᵀβ Φ + diag{α})⁻¹, CoFEM sets up *K* linear systems — each with a randomly drawn right-hand side — and solves them using **preconditioned conjugate gradient (CG)**, an iterative solver that requires only matrix-vector products, never the full matrix.

2. **Estimate the diagonal via the Rademacher trick.** The **Rademacher diagonal estimator** shows that averaging the outputs of random "probe" vectors fired through the system yields an unbiased estimate of the diagonal of the inverse covariance — no direct computation required. CoFEM replaces an *O(D²)* impossibility with a statistical estimate that converges reliably.

3. **Parallelize across GPUs.** Because the *K* linear systems are independent, they run simultaneously on GPU hardware — something most SBL competitors can't exploit due to memory constraints.

![Figure 1](/iaifi-research-blog/figures/2105_10439/figure_1.png)

Per-iteration cost drops from *O(D³)* to *O(UKτD)*, where *τD* is one matrix-vector multiply and *U, K* are small fixed hyperparameters. Memory drops from *O(D²)* to *O(DK)*. Crucially, for measurement matrices satisfying the **restricted isometry property (RIP)** — a standard condition in compressed sensing ensuring measurements capture the signal broadly — the authors prove *U* and *K* can stay fixed as *D* grows. You don't need more probes just because the problem gets bigger.
...
SBL's ability to quantify uncertainty has always been its defining feature — what makes it preferable to LASSO or ℓ₁-regularized compressed sensing. But that feature was locked behind a computational wall for any problem that mattered at scale. CoFEM tears down that wall.

![Figure 3](/iaifi-research-blog/figures/2105_10439/figure_2.png)

The paper demonstrates this with two real applications. In **calcium imaging deconvolution** — inferring neural spiking activity from fluorescence traces — CoFEM-equipped SBL recovers firing patterns with competitive accuracy while delivering the uncertainty estimates that purely deterministic methods can't provide. In **multi-contrast MRI reconstruction**, where the goal is to simultaneously reconstruct multiple tissue-contrast images from undersampled k-space data, CoFEM handles the enormous dimensionality of volumetric brain scans in practical compute times.

Both extensions — multi-task learning across MRI contrasts, non-negativity constraints for calcium signals — slot cleanly into the CoFEM framework, demonstrating its flexibility beyond the baseline setting.

![Figure 4](/iaifi-research-blog/figures/2105_10439/figure_2.png)

There's a broader template here. Many powerful Bayesian methods have remained confined to low-dimensional settings because their inference costs scale catastrophically. CoFEM shows the path: identify what you *actually* need from an expensive intermediate object, then estimate it cheaply using randomized numerical linear algebra. That approach could unlock Bayesian inference across scientific domains — gravitational wave analysis, particle physics inverse problems — wherever uncertainty quantification is exactly what researchers need but computational cost has made it prohibitive.
...
<div style="display:flex;gap:0.75rem;align-items:flex-start;padding:1rem;margin-bottom:0.75rem;border-radius:0.5rem;background:#f5f5f5;border:1px solid #d4d4d4;"><img src="/iaifi-research-blog/images/logo-fi-black.svg" alt="" style="width:32px;height:32px;flex-shrink:0;" /><div><strong style="color:#1a1a1a;">Interdisciplinary Research Achievement</strong><br/><span style="color:#374151;">This work bridges Bayesian statistics, numerical linear algebra, and signal processing, delivering a method directly applicable to scientific imaging — from MRI reconstruction to neural calcium imaging — where uncertainty quantification has genuine interpretive value.</span></div></div>
<div style="display:flex;gap:0.75rem;align-items:flex-start;padding:1rem;margin-bottom:0.75rem;border-radius:0.5rem;background:#eff6ff;border:1px solid #bfdbfe;"><img src="/iaifi-research-blog/images/logo-ai-blue.svg" alt="" style="width:32px;height:32px;flex-shrink:0;" /><div><strong style="color:#2c5f8a;">Impact on Artificial Intelligence</strong><br/><span style="color:#374151;">CoFEM demonstrates that randomized numerical methods can dramatically accelerate Bayesian inference, reducing covariance-matrix EM from cubic to near-linear complexity — a fundamental advance for scalable probabilistic machine learning.</span></div></div>
